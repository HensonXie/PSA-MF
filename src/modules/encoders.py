...
from .custom_bert import BertModelxx, BertConfigxx 

class LanguageEmbeddingLayer(nn.Module):
    def __init__(self, hp):
        super(LanguageEmbeddingLayer, self).__init__()
        self.bertmodel = BertModel.from_pretrained(hp.bert_path)

    def forward(self, sentences, bert_sent, bert_sent_type, bert_sent_mask):
        bert_output = self.bertmodel(input_ids=bert_sent,
                                     attention_mask=bert_sent_mask,
                                     token_type_ids=bert_sent_type)
        return bert_output[0], bert_output.hidden_states

class ssLanguageEmbeddingLayer(nn.Module):
    def __init__(self, hp):
        super(ssLanguageEmbeddingLayer, self).__init__()
        config = BertConfigxx.from_pretrained(hp.bert_path, output_hidden_states=True)
        self.bertmodel = BertModelxx.from_pretrained(hp.bert_path, config=config)

    def forward(self, sentences, bert_sent, bert_sent_type, bert_sent_mask, p_hidden_statess):
        bert_output, distill_loss_6 = self.bertmodel(
            input_ids=bert_sent,
            attention_mask=bert_sent_mask,
            token_type_ids=bert_sent_type,
            p_hidden_state=p_hidden_statess
        )
        return bert_output[0], bert_output.hidden_states, distill_loss_6

class PLanguageEmbeddingLayer(nn.Module):
    def __init__(self, hp):
        super(PLanguageEmbeddingLayer, self).__init__()
        self.bertmodel = BertModel.from_pretrained(hp.personality_bert_path, 
            output_hidden_states=True)

    def forward(self, sentences, bert_sent, bert_sent_type, bert_sent_mask, p_bert_sentences=None, p_bert_sentence_types=None, p_bert_sentence_att_mask=None):
        # 修复参数不匹配问题，确保只传需要的参数
        if p_bert_sentences is not None:
             # 如果这里是为了兼容接口，请确认实际使用的是 bert_sent 还是 p_bert_sentences
             # 假设使用的是 p_bert_sentences:
            bert_output = self.bertmodel(input_ids=p_bert_sentences,
                                        attention_mask=p_bert_sentence_att_mask,
                                        token_type_ids=p_bert_sentence_types)
        else:
             bert_output = self.bertmodel(input_ids=bert_sent,
                                        attention_mask=bert_sent_mask,
                                        token_type_ids=bert_sent_type)
...
class Clip(nn.Module):
    ...
    def forward(self, x, y):  # 32,768  32,64
        ...
        x = x / x.norm(dim=1, keepdim=True)#(128,128)
...
class RNNEncoder(nn.Module):
    ...
    def forward(self, x, lengths, ss):
        lengths = lengths.to(torch.int64)
        packed_sequence = pack_padded_sequence(x, lengths.cpu(), enforce_sorted=False)
        rnn_output, final_states = self.rnn(packed_sequence)
        encoder_outputs, _ = pad_packed_sequence(rnn_output, total_length=ss)
